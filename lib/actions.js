...
const searchFilteredTable = async (tableName, searchFields, query) => {
  ...
  // Null check and add the table name to each object in the result
  const dataWithTableName = data ? data.map((item) => ({ ...item, tableName })) : [];
...
// Search tag function for home page 5 results
